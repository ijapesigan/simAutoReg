...
//'
//' @details
//' The [SimVAR2()] function generates synthetic time series data
//' from a Vector Autoregressive (VAR) model.
...
arma::mat SimVAR2(int time, int burn_in, const arma::vec& constant,
  ...
  for (int t = p; t < total_time; t++) {
    ...
    for (int lag = 0; lag < p; lag++) {
      arma::mat lagged_data = data.cols(t - lag - 1, t - lag - 1);
      data.col(t) += coef.cols(lag * k, (lag + 1) * k - 1) * lagged_data;
    }

    data.col(t) += mult_noise;
  }
...
//' @export
// [[Rcpp::export]]
arma::mat SimVARExo2(int time, int burn_in, const arma::vec& constant,
                     const arma::mat& coef, const arma::mat& chol_cov,
                     const arma::mat& exo_mat, const arma::mat& exo_coef) {
  int k = constant.n_elem;  // Number of variables
  int p = coef.n_cols / k;  // Order of the VAR model (number of lags)
...
//'
//' @details
//' The [SimVARZIP2()] function generates synthetic time series data
...
//' The `SimVARZIP2` function goes further by using the generated values
